...
$(function () {
  ...
  function addrFn () {
    ...
    // province.innerHTML = `<option value=""> ---省份--- </option>` + map[0].map(item => `<option value="${item}"> ${item} </option>`).join('')
    let dom = '<option value=""> ---省份--- </option>'
    for(let key in map[0]) {
      dom = `${dom} <option value="${map[0][key]}"> ${map[0][key]} </option>`
    }
    province.innerHTML = dom
  }
  addrFn()
  ...
  function cityChangeFn (index){
    const city =document.querySelector('#city')
    let cityList = map[index]
    console.log(cityList);
    let dom = '<option value=""> ---城市--- </option>'
    for(let key in cityList) {
      console.log(cityList[key]);
      dom = `${dom} <option value="${cityList[key]}"> ${cityList[key]} </option>`
    }
    city.innerHTML = dom
  }
});
